...
sub pack {
   ...
   ||  ($self->pdu_type == NF_SNMP_PDUTYPE_REPORT)) {
      ...
      my $agent_addr;
      if ($self->agent_addr =~ /:/) {
         $agent_addr = inet6Aton($self->agent_addr)
      } else {
         $agent_addr = inetAton($self->agent_addr)
      }
      $raw = $snmpasn->encode(
         version   => $self->version,
         community => $self->community,
...
sub _inetNtoa {
    ...
    if ($Socket::VERSION >= 1.94) {
        ...
        if (defined($address)) {
            ...
            # print "getnameinfo($addr) failed - $err";
            return undef
        }
    } else {
        if (length($addr) == 4) {
            ...
            # Poor man's IPv6
            return join ':', (CORE::unpack '(a4)*', CORE::unpack ('H*', $addr))
        }
    }
}
